...
void initstring(dynstringptr dp, int capacity, int allocated)
{
    // inizializza una string gia' allocata
    dp->content = capacity > 0 ? malloc(sizeof(char) * capacity) : NULL;
    dp->capacity = capacity;
    dp->last = 0;
    dp->allocated = allocated;
}
...
char* tocharstar(dynstringptr p)  // non crea una copia!!!!
{
    *(p->content + p->last) = '\0';
    return p->content;
}
...
{
    // cancella stringa, il chiamante potrebbe perdere la stringa se non la ha salvata con strcpy()
    if(p->allocated) {
        free(p->content);
        free(p);
        return 1;
